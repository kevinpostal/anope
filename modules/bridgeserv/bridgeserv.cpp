	...
	, public BridgeCore
{
	...
	Server *EnsureLink(BridgeProtocol *protocol, const Anope::string &space)
	{
		...
		if (existing)
		{
			if (!existing->IsJuped())
			{
				Log(this) << "BridgeServ: " << name << " is already a real server on the network; not bridging it.";
				return nullptr;
			}
	...
	Anope::string MakeNick(const Anope::string &raw, const Anope::string &suffix) const
	{
		...
		for (unsigned counter = 2; User::Find(nick, true) || !IRCD->IsNickValid(nick); ++counter)
		{
			if (counter > 99)
				return "";
	...
	Anope::string MakeIdent(const Anope::string &user_id)
	{
		...
		SHA256(reinterpret_cast<const unsigned char *>(user_id.c_str()), user_id.length(), digest);
	...
	BridgeClient *EnsureClient(Bridge *bridge, const Anope::string &user_id, const Anope::string &display)
	{
		...
		if (nick.empty())
		{
			Log(this) << "BridgeServ: unable to allocate an IRC nick for " << protocol->GetName() << " user " << user_id << "; dropping message.";
			return nullptr;
		}
		...
		const Anope::string realname = Anope::Format("%s (%s)", display.c_str(), protocol->GetName().c_str());
		User *user = User::OnIntroduce(nick, this->MakeIdent(user_id), protocol->GetDomain(),
			"", "", link, realname, Anope::CurTime, "", IRCD->UID_Retrieve(), nullptr);
		if (!user)
		{
		...
		Log(this) << "BridgeServ: introduced pseudo client " << nick << "!" << user->GetIdent() << "@"
			<< user->host << " on " << link->GetName();
		return client;
	}
	...
	void PartClient(BridgeClient *client, const Anope::string &channel_name, const Anope::string &reason)
	{
	...
	void RemoveClient(BridgeClient *client, const Anope::string &reason, bool synced)
	{
		if (client->user)
		{
			if (synced)
...
public:
	...
	const std::vector<Bridge *> &GetBridges() const override { return this->bridges; }

	Bridge *FindRemote(const Anope::string &protocol, const Anope::string &channel) const override
	{
		for (auto *bridge : this->bridges)
		{
			if (bridge->protocol.equals_ci(protocol) && bridge->foreign_channel.equals_ci(channel))
				return bridge;
		}
		return nullptr;
	}
	...
	void SaveBridge(Bridge *bridge) override
	{
		bridge->QueueUpdate();
	}
	...
	void DeliverListing(const Anope::string &requester, const Anope::string &svc, bool channels, bool failed, const std::vector<Anope::string> &lines) override
	{
		...
		if (failed)
		{
			u->SendMessage(bi, channels
				? _("Failed to retrieve the channel list from the bridged network.")
				: _("Failed to retrieve the space list from the bridged network."));
			return;
		}
		...
		{
			u->SendMessage(bi, channels
				? _("No text channels were found in that space.")
	...
	const std::map<Anope::string, BridgeProtocol *, ci::less> &GetProtocols() const
	{
	...
	void ReapIdleUsers()
	{
		...
		{
			BridgeClient *client = it->second;
			if (!client->user || Anope::CurTime - client->last_active <= this->user_idle)
			{
				++it;
				continue;
			}
		...
		, reaper(this)
	{
		...
		Log(this) << "BridgeServ: Loaded with the " << discord->GetName() << " bridge protocol.";
	}
	...
	const Anope::string &space, const Anope::string &channel, const Anope::string &suffix)
{
	...
	{
		source.Reply(_("\002%s\002 can not be used as a nickname suffix on this network. "
			"Characters outside the usual nickname alphabet have to be allowed with "
			"\002networkinfo:nick_chars\002 first."), suffix.c_str());
		return nullptr;
	}
	return protocol;
}
...
CommandBSAdd::CommandBSAdd(ModuleBridgeServ *creator)
	: Command(creator, "bridgeserv/add", 3, 4)
	, module(creator)
{
	this->SetDesc(_("Bridge an IRC channel to a channel on a bridged network"));
	this->SetSyntax(_("\037#channel\037 \037space-id\037 \037channel-id\037 [\037nick-suffix\037]"));
}
...
void CommandBSAdd::Execute(CommandSource &source, const std::vector<Anope::string> &params)
{
	...
	if (const auto *other = this->module->FindRemote(protocol->GetName(), foreign_channel))
	{
		source.Reply(_("That channel is already bridged to \002%s\002."), other->irc_channel.c_str());
		return;
	}
...
bool CommandBSAdd::OnHelp(CommandSource &source, const Anope::string &subcommand)
{
	...
	source.Reply(" ");
	source.Reply(_(
		"Creates a bridge between an IRC channel and a channel on a "
	...
	source.Reply(" ");
	source.Reply(_(
		"If a nickname suffix is given then it is appended to the nick of "
...
CommandBSSet::CommandBSSet(ModuleBridgeServ *creator)
	: Command(creator, "bridgeserv/set", 3, 4)
	, module(creator)
{
...
void CommandBSSet::Execute(CommandSource &source, const std::vector<Anope::string> &params)
{
	...
	const auto *other = this->module->FindRemote(protocol->GetName(), foreign_channel);
...
bool CommandBSDel::OnHelp(CommandSource &source, const Anope::string &subcommand)
{
	...
	source.Reply(" ");
	source.Reply(_(
		"Removes an existing bridge, deletes its delivery endpoint, and "
...
void CommandBSList::Execute(CommandSource &source, const std::vector<Anope::string> &params)
{
	...
	list.AddColumn(_("Channel")).AddColumn(_("Network")).AddColumn(_("Space")).AddColumn(_("Remote channel"))
		.AddColumn(_("Suffix")).AddColumn(_("Endpoint")).AddColumn(_("Users")).AddColumn(_("Nicks"));
...
bool CommandBSList::OnHelp(CommandSource &source, const Anope::string &subcommand)
{
	...
	source.Reply(" ");
	source.Reply(_(
		"Lists the configured bridges: the network and channel each one is "
...
bool CommandBSGuilds::OnHelp(CommandSource &source, const Anope::string &subcommand)
{
	...
	source.Reply(" ");
	source.Reply(_(
		"Lists the spaces (Discord guilds) which the bridge account has "
...
bool CommandBSChannels::OnHelp(CommandSource &source, const Anope::string &subcommand)
{
	...
	source.Reply(" ");
	source.Reply(_(
		"Lists the text channels of a space as \037name\037 (\037id\037). "
